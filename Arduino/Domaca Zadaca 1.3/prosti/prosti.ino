...
{
  if (n < 2) return 0;
  for (uint16_t i = 2; i < n; i++) if (n % i == 0) return 0;
  return 1;
}
...
void loop() {
  ...
  while (counter > 0)
  {
    ...
    {
      delay(500);
      uint16_t prost_naop = naop(counter);
      while (prost_naop > 0)
      {
        for (byte i = 0; i < 8; i++) sendData(i + 1, pgm_read_byte_near(&FontA[(prost_naop % 10) + '0'][i]));
        prost_naop /= 10;
        delay(300);
